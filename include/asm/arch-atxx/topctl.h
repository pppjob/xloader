...
#endif
static void topctl_write_reg(uint32_t reg, uint32_t val)
{
	writel(val, ATXX_TOPCTL_BASE + reg);
}

static uint32_t topctl_read_reg(uint32_t reg)
{
	return readl(ATXX_TOPCTL_BASE + reg);
}

static void topctl_set_bits(uint32_t reg, uint32_t bits)
{
	set_bit(bits, (volatile void *)(ATXX_TOPCTL_BASE + reg));
}

static void topctl_clear_bits(uint32_t reg, uint32_t bits)
{
